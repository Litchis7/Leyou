...
public class SearchService {
    ...
    public Goods buildGoods(Spu spu) throws IOException {
        ...
        //根据分类的id查询分类名称
        List<String> namesByIds = this.categoryClient.queryNamesByIds(Arrays.asList(spu.getCid1(), spu.getCid2(), spu.getCid3()));
        ...
        skus.forEach(sku->{
            ...
            //直接将skumaplist放进去字段太多了，只要4个字段就够了
        ...
        /*Map<String,Object> genericSpecMap = (Map<String, Object>) MAPPER.reader(spuDetail.getGenericSpec(),new TypeReference<Map<String,Object>>(){});*/
       //把通用的规格参数值，进行反序列化;规格参数是json数据，构造对象不方便，使用Map接受
        ...
        specParams.forEach(specParam -> {
            ...
            if (specParam.getGeneric()){
                //如果是通用类型的参数，从genericSpecMap获取规格参数;Id为Long类型，Map为<string>需要转化为string类型
                //Object object = genericSpecMap.get(specParam.getId().toString()); //object不太好，用string字符串方便
        ...
        goods.setAll(spu.getTitle() + " " + StringUtils.join(namesByIds," ") + " " + brand.getName());
        //获取spu下的所有sku价格
    ...
    private String chooseSegment(String value, SpecParam p) {
        ...
        for (String segment : p.getSegments().split(",")) {
            ...
            double end = Double.MAX_VALUE;//2^64
            if(segs.length == 2){
